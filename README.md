...
- Se trata de un problema de negocio real de una entidad bancaria. El repo es un **clon del original anonimizando** la documentación publicable y eliminando aquellas partes que no son relevantes para este proyecto por su caracter confidencial.

- **La funcionalidad para validar el comportamiento del motor de ofertas en el sistema de *workflow* del banco no es accesible**. Esta funcionalidad se ha utilizado para crear pruebas automatizadas mediante un servicio web que contrastan los valores esperados y obtenidos por el motor/reglas/parámetros implementado en el sistema de la entidad con el motor desarrollado en este proyecto y que se ha certificado.
...
Todo el rastro queda **versionado en `openspec/`**. A día de hoy hay **15 cambios completados de extremo a extremo**, entre ellos: despliegue al sistema de workflow, mapeo de ofertas WF, *snapshots* y restauración, vigencias por fecha/datetime, panel y correcciones del simulador, periodo activo de parámetros, la página de ofertas con cascada de periodos, caché de configuración por *fingerprint*/TTL, borrado en cascada de ofertas y el endurecimiento de seguridad **RBAC + salvaguardas de configuración** (detallado más abajo).
...
### La seguridad como parte del proceso, no como un añadido de última hora

Antes de la entrega se realizó una **auditoría OWASP Top 10 (2021)** completa sobre el código (`rule_set/docs/seguridad/informe-owasp-top10-2026-07-10.md`): 5 revisiones estáticas en paralelo, sin explotación real, que catalogaron 16 hallazgos con severidad e identificador estable (`OWASP-NN`) para poder planificar su corrección vía SDD como cualquier otra funcionalidad.

Los 2 hallazgos **críticos** —control de acceso roto (el rol venía en el JWT pero ningún controlador lo comprobaba) y reemplazo total de la configuración sin salvaguardas— junto con uno **alto** (restauración de *snapshots* sin verificación de integridad) se resolvieron mediante un cambio SDD completo (`rbac-and-config-safeguards`, 4 PRs encadenados, en TDD estricto), atravesando el mismo flujo explore→proposal→spec→design→tasks→apply→verify→archive:

- **RBAC efectivo** (OWASP-01): middleware `requireRole("admin")` montado sobre `/api/admin/*`; catálogo `ALLOWED_ROLES` (`admin`/`viewer`); el frontend oculta la navegación de administración y gestiona el 403 sin desautenticar al usuario.
...
> **Estado actual de la suite:** `339 tests`. El motor de reglas, los escenarios de negocio, los
> validadores y los adaptadores puros (no requieren base de datos) **pasan siempre: 301 pass, 0 fail**.
> Las 38 pruebas restantes son de **integración** y necesitan una instancia real de SQL Server
> (POC y/o el sistema de Workflow del banco) para ejecutarse: 36 fallan por falta de conexión sin
> esa infraestructura —no son fallos de código— y 2 se omiten (`skipped`) explícitamente por diseño
> cuando no hay credenciales del sistema Workflow (`WF_TOKEN`/`WF_BASE_URL`). Con la infraestructura
> disponible (`docker compose up` o credenciales reales en `api/.env`) las 36 pasan correctamente,
> como recoge el proceso de evidencias (`docs/evidencias/`, generado en cada entrega significativa
> siguiendo `CLAUDE.md`).
...
## 6. Autenticación y control de acceso (JWT + RBAC)

La aplicación está protegida con **autenticación por token JWT** (JSON Web Token) y **control de acceso por rol (RBAC)** sobre las rutas de administración.
...
Cada usuario tiene un `role` (`admin` o `viewer`, catálogo `ALLOWED_ROLES`) embebido en el JWT. El middleware `requireRole("admin")` se monta únicamente sobre `/api/admin/*` (ofertas, reglas, parámetros, *snapshots*, *apply*): un `viewer` autenticado recibe `403` en esas rutas, pero conserva sesión y acceso a simulación (`/api/simulate/*`) y a la consulta de elegibilidad de workflow (`/api/workflow/*`), que no exige rol. En el frontend, `auth.service.ts` decodifica el rol del propio JWT para ocultar la navegación de administración a los `viewer` sin necesidad de una llamada adicional al backend.
