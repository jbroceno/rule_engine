...
1. **MOTOR_FECHAS** — gestión centralizada de períodos de vigencia de reglas y parámetros.
2. **Mejoras al sistema de snapshots** — restauración a POC o Workflow, snapshots del entorno Workflow.
3. **Publicación en Workflow** — transformación del modelo POC al modelo MRO_ de Workflow.
...
- Las tablas `MRO_MOTORREGLA` y `MRO_MOTORPARAM` conservan sus columnas `VIGENCIA_DESDE_DT`/`VIGENCIA_HASTA_DT` para compatibilidad con el procesamiento nativo de Workflowblicar, esas columnas se populan desde `MOTOR_FECHAS`.
...
### RF-001 — Tabla de períodos de vigencia (MOTOR_FECHAS)
...
| `valid_from` | DATE NOT NULL | Inicio del período (inclusive). |
| `valid_to` | DATE NULL | Fin del período (inclusive). NULL = sin fin. |
...
### RF-002 — Asociación de reglas al período de vigencia
...
### RF-003 — Asociación de parámetros al período de vigencia
...
### RF-004 — Vista de línea de tiempo en el configurador
...
### RF-005 — CRUD de períodos de vigencia

El usuario DEBERÁ poder crear, editar y eliminar registros en `MOTOR_FECHAS` desde la UI.

- **Crear**: formulario con Desde, Hasta (opcional), Tipo y Descripción.
- **Editar**: formulario pre-rellenado. No se permitirá editar un período si tiene reglas o parámetros asociados en uso activo (se deberá mostrar error descriptivo).
- **Eliminar**: bloqueado si existen reglas o parámetros referenciando el registro (error 409).

### RF-006 — Selector de período al gestionar reglas y parámetros
...
### RF-007 — Columna de entorno en la tabla de snapshots
...
### RF-008 — Restauración de snapshot con destino seleccionable
...
### RF-009 — Lógica de restauración de snapshot en Workflow

Cuando el destino es Workflow y el usuario confirma la operación:

1. El sistema DEBERÁ localizar y eliminar las reglas (`MRO_MOTORREGLA` con sus condiciones, valores y acciones) y parámetros (`MRO_MOTORPARAM`) del entorno Workflow cuya `VIGENCIA_DESDE_DT` y `VIGENCIA_HASTA_DT` coincidan con el rango destino indicado.
2. Transformar y escribir las reglas y parámetros del snapshot en el modelo MRO_, populando `VIGENCIA_DESDE_DT` y `VIGENCIA_HASTA_DT` con el rango destino indicado.
...
### RF-010 — Publicación en Workflow ("Publicar en Workflow")
...
2. Solicite el rango de fechas destino en Workflow (desde-hasta). Este rango poblará `VIGENCIA_DESDE_DT`/`VIGENCIA_HASTA_DT` en las tablas MRO_.
3. Cree automáticamente un snapshot de seguridad del estado actual del entorno Workflow antes de aplicar.
4. Aplique la misma lógica de RF-009 (eliminar datos del período destino si existen, y escribir desde el origen POC).

### RF-011 — Snapshot del entorno Workflow
...
### RF-012 — Endpoint adaptador del servicio Workflow
...
**Request** — contrato del servicio de Workflow más campo adicional `domiciliaNomina` requerido por el adaptador:
```json
{
  "token": "...",
...
### RF-013 — Tests automáticos de comparación
...
El endpoint `/api/workflow/condiciones-hipotecas` DEBERÁ requerir el campo `token` en el body del request. La validación del token queda delegada al sistema Workflow y fuera del scope de la POC.
...
| RNF-004 | Compatibilidad Workflow | Las columnas `VIGENCIA_DESDE_DT`/`VIGENCIA_HASTA_DT` de las tablas MRO_ DEBEN mantenerse y popularse correctamente en toda operación de escritura para no romper el procesamiento nativo de Workflow. |
