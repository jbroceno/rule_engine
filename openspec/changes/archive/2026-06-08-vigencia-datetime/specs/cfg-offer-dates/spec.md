# Especificación delta — vigencia-datetime / cfg-offer-dates
...
> siguientes apartados de `openspec/specs/cfg-offer-dates/spec.md`:
> - Los tipos de dato de `valid_from` y `valid_to` (DATE → DATETIME2(0)).
> - Las reglas de validación de solapamiento y orden de fechas (ahora a granularidad
>   de segundos).
> - El comportamiento del formulario de edición (preservación del componente de hora).
> - El comportamiento del formulario de creación (captura de fecha y hora).
>
> El resto de requisitos (`RF-001` a `RF-004` del spec base, nomenclatura de tabla y
> clave primaria, propagación a módulos, migración de renombrado) permanece sin cambio.
...
3. [RF-COD-01 — Tipo datetime en cfg_offer_dates](#3-rf-cod-01--tipo-datetime-en-cfg_offer_dates)
4. [RF-COD-02 — Validación de solapamiento a granularidad de segundos](#4-rf-cod-02--validacion-de-solapamiento)
5. [RF-COD-03 — Edición preserva componente de hora](#5-rf-cod-03--edicion-preserva-hora)
6. [RF-COD-04 — Comparación temporal, no léxica](#6-rf-cod-04--comparacion-temporal)
7. [RF-COD-05 — Frontend datetime-local con step=1](#7-rf-cod-05--frontend-datetime-local)
8. [RF-COD-06 — Migración de esquema DATE → DATETIME2(0)](#8-rf-cod-06--migracion-de-esquema)
...
| Requisito base | Cambio |
|----------------|--------|
| `cfg_offer_dates.valid_from/valid_to` tipo `DATE` | MODIFICADO → `DATETIME2(0)` |
| Validación de solapamiento día-granular | MODIFICADO → granularidad de segundos |
| Cierre de período: `valid_to = valid_from - 1 día` | MODIFICADO → `valid_to = valid_from del período siguiente` exacto |
| Formulario edición: `substring(0,10)` al leer/escribir fecha | MODIFICADO → envío y recepción de datetime completo |
| Comparación `valid_to <= valid_from` como string | MODIFICADO → comparación temporal de datetime |
| `<input type="date">` en frontend | MODIFICADO → `<input type="datetime-local" step="1">` |
| Pipe de presentación `dd/MM/yyyy` | MODIFICADO → `dd/MM/yyyy HH:mm:ss` |

### Lo que NO cambia

- Nombre de tabla (`dbo.cfg_offer_dates`), clave primaria (`offer_date_id`), FK desde reglas y parámetros.
- Semántica de solapamiento: sigue prohibido para períodos del mismo tipo.
- Lógica de la UI (crear, editar, eliminar, bloqueo por dependencias).
- Rutas HTTP (`/api/admin/offer-dates`).
- Idempotencia del script de migración de renombrado.
...
## 3. RF-COD-01 — Tipo datetime en cfg_offer_dates

El sistema SHALL almacenar `valid_from` y `valid_to` de `dbo.cfg_offer_dates` con tipo
`DATETIME2(0)`, soportando valores no-medianoche (por ejemplo `2026-03-15 14:32:07`).

#### Scenario: Período con hora no-medianoche creado desde la API
...
- **And** la columna `valid_from` del tipo `DATETIME2` NO muestra milisegundos (precesión = 0)

#### Scenario: Período con medianoche — compatibilidad hacia atrás
...
## 4. RF-COD-02 — Validación de solapamiento a granularidad de segundos

El sistema SHALL detectar solapamiento de períodos comparando `valid_from` y `valid_to`
a granularidad de segundos. Dos períodos se solapan si sus intervalos `[valid_from, valid_to)`
se intersectan en al menos un segundo.

#### Scenario: Período adyacente (sin solapamiento al segundo)
...
#### Scenario: Solapamiento de un segundo
...
#### Scenario: Solapamiento que solo sería visible al segundo (no al día)
...
## 5. RF-COD-03 — Edición preserva componente de hora

El sistema SHALL preservar el componente de hora de `valid_from` y `valid_to` al editar
un período de vigencia existente. La operación de edición no debe truncar ni descartar
la porción de hora del valor original.

#### Scenario: Edición de descripción preserva hora exacta
...
#### Scenario: Edición de valid_to envía datetime completo
...
## 6. RF-COD-04 — Comparación temporal, no léxica

El validador del backend SHALL comparar `valid_to` vs `valid_from` usando comparación
temporal de datetime, no comparación léxica de strings.

#### Scenario: valid_to menor que valid_from en datetime — rechazado
...
#### Scenario: valid_to igual a valid_from — rechazado
...
#### Scenario: valid_to posterior a valid_from — aceptado
...
#### Scenario: Comparación no se confunde con orden lexicográfico
...
## 7. RF-COD-05 — Frontend datetime-local con step=1

El sistema SHALL usar `<input type="datetime-local" step="1">` para todos los campos
de fecha de vigencia en la interfaz de usuario. Los campos NO deben usar `type="date"`.

La UI SHALL mostrar los valores de `valid_from` y `valid_to` con el formato
`dd/MM/yyyy HH:mm:ss` en modo de solo lectura (columnas de tabla y vistas de detalle).

#### Scenario: Input captura segundos
...
#### Scenario: Tabla lista muestra hora completa
...
## 8. RF-COD-06 — Migración de esquema DATE → DATETIME2(0)

El sistema SHALL incluir un script de migración `rule_set/sql/migration_vigencia_datetime.sql`
que altere las columnas `valid_from` y `valid_to` de `dbo.cfg_offer_dates` de `DATE`
a `DATETIME2(0)`. La migración es idempotente y backward-compatible: los valores `DATE`
existentes se convierten a la hora `00:00:00` (medianoche).

#### Scenario: Migración sobre instancia con valores DATE existentes

- **Given** la tabla `dbo.cfg_offer_dates` con columnas `DATE` y filas con `valid_from = 2026-01-01`
- **When** se ejecuta `migration_vigencia_datetime.sql`
...
#### Scenario: Migración idempotente (segunda ejecución)

- **Given** la tabla `dbo.cfg_offer_dates` ya con columnas `DATETIME2(0)`
- **When** se ejecuta `migration_vigencia_datetime.sql` por segunda vez
...
| CA-COD-008 | Migración | Idempotente | Ejecutar `migration_vigencia_datetime.sql` dos veces | Sin error en la segunda ejecución |
