...
- [x] Actualizar `rule_set/rules.json`: 5 reglas con nombre `"INIT Rechazo: No es primera vivienda habitual"`, field `primeraViviendaHabitual`, operador `EQ` con `value1: 0`, motivo `NO_PRIMERA_VIVIENDA`.
...
- [x] Editar `rule_set/web/src/app/pages/init-simulator-page.component.html`: actualizar opciones de `<select>` `tipoAlta` a `ALTA_NUEVA`, `NOVACION`, `NOVACIÓN_RDL`, `SUBROGACION`.
- [x] Editar `rule_set/web/src/app/pages/init-simulator-page.component.ts`: defaults `tipoAlta = "ALTA_NUEVA"`, `finalidad = 1`.
