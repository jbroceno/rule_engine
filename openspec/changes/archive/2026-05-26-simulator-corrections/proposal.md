...
Archivos afectados:
- `rule_set/rules.json` — 5 reglas actualizadas
...
Eliminados selector de número de titulares y fieldset Titular 2. `submit()` hardcodea `numTitulares: 1` y los campos T2 a `0/false`. Defaults actualizados: `tipoAlta = "ALTA_NUEVA"`, `finalidad = 1`. El checkbox `primeraViviendaHabitual` reemplaza a `tieneOtrasPropiedades`.
...
- `TIPO_ALTA_ADMITIDAS` actualizado a `'["CAPTACION","NOVACION"]'`
