...
Given un solicitante con primeraViviendaHabitual = 1
  And el resto de campos cumplen las condiciones de la oferta diferente a FIDELIZACION
...
**Descripción**: los tres simuladores (INIT, PRE, FINAL) ofrecen únicamente los tipos de alta válidos para Ofertas Hipotecarias: `CAPTACION`, `NOVACION`.

**Criterios**:
- El control `tipoAlta` del formulario es un `<select>` con esas dos opciones, en ese orden.
- El valor `NUEVA` (antiguo) ya no aparece en ningún formulario ni en el seed.
- El parámetro `TIPO_ALTA_ADMITIDAS` del seed SQL contiene `["CAPTACION","NOVACION"]` para las ofertas distintas a FIDELIZACION. `SUBROGACION` puede llegar como valor de `tipoAlta` desde otros canales (p. ej. WF), pero no es una opción seleccionable en el `<select>`.
...
Then ve únicamente las opciones CAPTACION, NOVACION
...
```
Given un input con tipoAlta = "CAPTACION"
When se ejecuta initcheck() contra una oferta distinta a FIDELIZACION sembrada con TIPO_ALTA_ADMITIDAS = ["CAPTACION","NOVACION"]
...
- `TIPO_ALTA_ADMITIDAS` se siembra con `'["CAPTACION","NOVACION"]'` para las 5 ofertas distintas a FIDELIZACION.
